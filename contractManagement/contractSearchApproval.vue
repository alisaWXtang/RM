<template>
  <div class="page-home">
    ...
    <el-card class="workflow-tab">
      ...
      <div v-if="contractIdVersionOption.length>0" class="workflow-tab-title history">
        <span>历史版本：</span>
        <el-select v-model="contractIdVersion" placeholder="请选择" value-key="version" @change="changeContractIdVersion">
          <el-option
            v-for="item in contractIdVersionOption"
            :key="item.version"
            :label="item.version"
            :value="item"
          />
      ...
      <el-tabs v-model="activeName" :class="{'changeTransform':tabs.length<3,'changeTransformF':tabs.length===2}" @tab-click="changeTab">
      ...
      <div v-if="showInit && showTip " style="text-align: center;line-height: 200px;">无查看权限</div>
    </el-card>
  </div>
</template>
...
import {
  getDocPermission, getDocPermissionContract, queryContractPresentPersonnelInfoById, queryContractBaseInfoPastRecordsByContractId
...
export default {
  ...
  async created() {
    ...
    if (this.$route.query.demandStageId === '3') {
      if (this.$route.query.isEdit !== 'F') {
        await queryContractPresentPersonnelInfoById({ contractId }).then(result => {
          this.contractIsEdit = result.data.edit
        }).catch()
      } else {
        this.contractIsEdit = true
      }
    }
    // this.contractIsEdit = this.$route.query.demandStageId === '3' && result.data.edit  /this.$route.query.isEdit
  ...
  mounted() {
    window.addEventListener('resize', this.resizeAddListener)
  },
  destroyed() {
    if (document.querySelector('.el-tabs__nav-next')) {
      document.querySelector('.el-tabs__nav-next').removeEventListener('click', this.listenerClick)
    }
  },
  ...
  methods: {
    resizeAddListener() {
      ...
      document.querySelector('.el-tabs__nav-next').addEventListener('click', this.listenerClick)
    },
    listenerClick() {
      const elTabsNnav = document.querySelector('.el-tabs__nav')
      const elTabsNavScroll = document.querySelector('.el-tabs__nav-scroll').clientWidth
      if (elTabsNavScroll - elTabsNnav.scrollWidth === parseInt(elTabsNnav.style.transform.match(/\(([^)]*)\)/)[1])) {
        elTabsNnav.style.transform = `translateX(${parseInt(elTabsNnav.style.transform.match(/\(([^)]*)\)/)[1]) - 40}px)`
      }
    },
    async queryContractBaseInfoPastRecordsByContractId() {
      const result = await queryContractBaseInfoPastRecordsByContractId({ id: this.$route.query.contractId, superAdmin: !!this.$route.query.approve })
      this.contractIdVersionOption = result.data.map(item => {
        if (!item.version) {
          item.version = '当前版本'
        }
        return item
      })
      this.contractIdVersion = result.data[0]
    },
    async changeTab(tab, event) {
      ...
      this.contractIdVersionOption = []
      if (this.demandStageId === '3') {
        this.contractIsEdit = false
        if (this.tabs[Number(this.activeName)].status) {
          this.queryContractBaseInfoPastRecordsByContractId()
        }
      ...
      if (this.demandStageId === '3' && !this.contractIsEdit) {
        ...
        queryContractBaseInfoList(params).then(result => {
          ...
          this.tabs = result.data.map((item, index) => {
            return this.setTab(index, item.approvelStatus, item.id, item.contractNumber)
          })
          if (this.tabs[0] && this.tabs[0].status) {
            this.queryContractBaseInfoPastRecordsByContractId()
          }
          this.showInit = true
          setTimeout(() => {
            this.resizeAddListener()
          }, 1000)
        })
      } else {
    ...
    // 设置tab值
    setTab(index, value, id, contractNumber) {
      const tab = {
      ...
      if (this.demandStageId === '3' && (value === '4')) {
        tab.requirementTabName = `合同审批${contractNumber}（已完成）`
        tab.status = value
      }
      if (this.demandStageId === '3') {
        tab.id = id
    ...
    async getSteps(id) {
      ...
      const params = {
        ...
        contractId: this.$route.query.isEdit === 'F' ? '' : id || this.$route.query.contractId,
        ...this.templateData
    ...
    changeContractIdVersion(val) {
      const contractId = val.id
      this.$router.push({
        path: this.$route.path,
        query: { ...this.$route.query, contractId }
      })
      this.$refs.contract[0].getContractDetail(val.version === '当前版本')
    }
  }
...
.mt-30{
  ...
  .changeTransformF /deep/.el-tabs__nav-scroll {
  ...
  .changeTransform /deep/.el-tabs__item.is-active {
    color: #000;
    font-weight: 600;
  }
